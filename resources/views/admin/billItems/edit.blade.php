...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bill-items.update", [$billItem->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_unit') ? 'is-invalid' : '' }}" type="number" name="total_unit" id="total_unit" value="{{ old('total_unit', $billItem->total_unit) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $billItem->amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bill') ? 'is-invalid' : '' }}" name="bill_id" id="bill_id" required>
                    @foreach($bills as $id => $entry)
                        <option value="{{ $id }}" {{ (old('bill_id') ? old('bill_id') : $billItem->bill->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('bill'))
                    <div class="invalid-feedback">
