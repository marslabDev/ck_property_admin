...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bill-histories.update", [$billHistory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="paid_by_id">{{ trans('cruds.billHistory.fields.paid_by') }}</label>
                <select class="form-control select2 {{ $errors->has('paid_by') ? 'is-invalid' : '' }}" name="paid_by_id" id="paid_by_id" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $billHistory->amount) }}" step="0.01" required>
