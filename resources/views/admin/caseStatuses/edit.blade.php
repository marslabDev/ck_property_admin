...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.case-statuses.update', [$caseStatus->id]) }}"
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name"
                    id="name" value="{{ old('name', $caseStatus->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('status_linking') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="status_linking" id="status_linking" value="1"
                        {{ $caseStatus->status_linking || old('status_linking', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="card d-none" id="status_linking_field">
                <div class="card-body">
                    <div class="form-group">
                        <label class="required" for="complaint_status_id">{{ trans('cruds.caseStatus.fields.complaint_status') }}</label>
                        <select class="form-control select2 {{ $errors->has('complaint_status') ? 'is-invalid' : '' }}"
                            ...
                            <option value="{{ $id }}" {{ (old('complaint_status_id') ? old('complaint_status_id') :
                                $caseStatus->complaint_status->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}
                            </option>
                            @endforeach
                        </select>
