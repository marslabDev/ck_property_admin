...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.manage-prices.update', [currentArea(), $managePrice->id]) }}"
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('house_type') ? 'is-invalid' : '' }}"
                    ...
                    <option value="{{ $id }}" {{ (old('house_type_id') ? old('house_type_id') : $managePrice->
                        house_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price_per_sq_ft') ? 'is-invalid' : '' }}" type="number"
                    name="price_per_sq_ft" id="price_per_sq_ft"
                    value="{{ old('price_per_sq_ft', $managePrice->price_per_sq_ft) }}" step="0.01" required>
