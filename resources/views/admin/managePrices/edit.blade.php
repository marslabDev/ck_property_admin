...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.managePrice.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.manage-prices.update", [$managePrice->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('area') ? 'is-invalid' : '' }}" name="area_id" id="area_id" required>
                    @foreach($areas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('area_id') ? old('area_id') : $managePrice->area->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('area'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('house_type') ? 'is-invalid' : '' }}" name="house_type_id" id="house_type_id" required>
                    @foreach($house_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('house_type_id') ? old('house_type_id') : $managePrice->house_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price_per_sq_ft') ? 'is-invalid' : '' }}" type="number" name="price_per_sq_ft" id="price_per_sq_ft" value="{{ old('price_per_sq_ft', $managePrice->price_per_sq_ft) }}" step="0.01" required>
