...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.paymentCharge.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.payment-charges.update", [$paymentCharge->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="particular">{{ trans('cruds.paymentCharge.fields.particular') }}</label>
                <input class="form-control {{ $errors->has('particular') ? 'is-invalid' : '' }}" type="text" name="particular" id="particular" value="{{ old('particular', $paymentCharge->particular) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\PaymentCharge::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $paymentCharge->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $paymentCharge->amount) }}" step="0.01" required>
