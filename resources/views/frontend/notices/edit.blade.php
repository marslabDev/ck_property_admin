    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.notices.update", [$notice->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="title_name">{{ trans('cruds.notice.fields.title_name') }}</label>
                            <input class="form-control" type="text" name="title_name" id="title_name" value="{{ old('title_name', $notice->title_name) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="detail" id="detail" required>{{ old('detail', $notice->detail) }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="people_in_role_id" id="people_in_role_id">
                                @foreach($people_in_roles as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('people_in_role_id') ? old('people_in_role_id') : $notice->people_in_role->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="people_in_area_id" id="people_in_area_id">
                                @foreach($people_in_areas as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('people_in_area_id') ? old('people_in_area_id') : $notice->people_in_area->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
...
<script>
    ...
    url: '{{ route('frontend.notices.storeMedia') }}',
    maxFilesize: 2, // MB
