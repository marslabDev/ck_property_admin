        ...
        <div class="col-md-12">
            @can('article_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.articles.create') }}">
                            ...
                            <tbody>
                                @foreach($articles as $key => $article)
                                    <tr data-entry-id="{{ $article->id }}">
                                        ...
                                        <td>
                                            @can('article_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.articles.show', $article->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('article_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.articles.edit', $article->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('article_delete')
                                                <form action="{{ route('frontend.articles.destroy', $article->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('article_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.articles.massDestroy') }}",
