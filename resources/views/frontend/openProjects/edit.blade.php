    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.open-projects.update", [$openProject->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="name">{{ trans('cruds.openProject.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $openProject->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="description" id="description">{{ old('description', $openProject->description) }}</textarea>
                        ...
                        <div class="form-group">
                            <label class="required" for="documents">{{ trans('cruds.openProject.fields.documents') }}</label>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control date" type="text" name="start_date" id="start_date" value="{{ old('start_date', $openProject->start_date) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control date" type="text" name="end_date" id="end_date" value="{{ old('end_date', $openProject->end_date) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="status" id="status" required>
                                ...
                                @foreach(App\Models\OpenProject::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $openProject->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
...
Dropzone.options.documentsDropzone = {
    url: '{{ route('frontend.open-projects.storeMedia') }}',
    maxFilesize: 10, // MB
    addRemoveLinks: true,
    headers: {
