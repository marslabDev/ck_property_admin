        ...
        <div class="col-md-12">
            @can('payment_item_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.payment-items.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-PaymentItem">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $paymentItem->id }}">
                                        ...
                                        <td>
                                            @can('payment_item_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.payment-items.show', $paymentItem->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('payment_item_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.payment-items.edit', $paymentItem->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('payment_item_delete')
                                                <form action="{{ route('frontend.payment-items.destroy', $paymentItem->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('payment_item_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.payment-items.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-PaymentItem:not(.ajaxTable)').DataTable({ buttons: dtButtons })
