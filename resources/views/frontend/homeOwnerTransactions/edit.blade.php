        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.edit') }} {{ trans('cruds.homeOwnerTransaction.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.home-owner-transactions.update", [$homeOwnerTransaction->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="user_id" id="user_id" required>
                                @foreach($users as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $homeOwnerTransaction->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('user'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="house_id" id="house_id" required>
                                @foreach($houses as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('house_id') ? old('house_id') : $homeOwnerTransaction->house->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('house'))
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="payment_plan_id" id="payment_plan_id" required>
                                @foreach($payment_plans as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('payment_plan_id') ? old('payment_plan_id') : $homeOwnerTransaction->payment_plan->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="payment_type_id" id="payment_type_id" required>
                                @foreach($payment_types as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('payment_type_id') ? old('payment_type_id') : $homeOwnerTransaction->payment_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="amount_paid" id="amount_paid" value="{{ old('amount_paid', $homeOwnerTransaction->amount_paid) }}" step="0.01" required>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="changes" id="changes" value="{{ old('changes', $homeOwnerTransaction->changes) }}" step="0.01" required>
