        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.create') }} {{ trans('cruds.supplierProposal.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.supplier-proposals.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="representative_name">{{ trans('cruds.supplierProposal.fields.representative_name') }}</label>
                            <input class="form-control" type="text" name="representative_name" id="representative_name" value="{{ old('representative_name', '') }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="open_project_id" id="open_project_id" required>
                                @foreach($open_projects as $id => $entry)
                                    <option value="{{ $id }}" {{ old('open_project_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
