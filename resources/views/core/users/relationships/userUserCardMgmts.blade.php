@can('user_card_mgmt_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('core.user-card-mgmts.create') }}">
                        ...
                        <tr data-entry-id="{{ $userCardMgmt->id }}">
                            ...
                            <td>
                                @can('user_card_mgmt_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('core.user-card-mgmts.show', $userCardMgmt->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('user_card_mgmt_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('core.user-card-mgmts.edit', $userCardMgmt->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('user_card_mgmt_delete')
                                    <form action="{{ route('core.user-card-mgmts.destroy', $userCardMgmt->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('user_card_mgmt_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('core.user-card-mgmts.massDestroy') }}",
