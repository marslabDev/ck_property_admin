...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("core.users.update", [$user->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control phone_no_mask {{ $errors->has('phone_no') ? 'is-invalid' : '' }}" type="text" name="phone_no" id="phone_no" value="{{ old('phone_no', $user->phone_no) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('areas') ? 'is-invalid' : '' }}" name="areas[]" id="areas" multiple required>
                    @foreach($areas as $id => $area)
                        <option value="{{ $id }}" {{ (in_array($id, old('areas', [])) || $user->areas->contains($id)) ? 'selected' : '' }}>{{ $area }}</option>
                    @endforeach
                </select>
                @if($errors->has('areas'))
                    <div class="invalid-feedback">
